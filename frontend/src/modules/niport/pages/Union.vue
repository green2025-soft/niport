...
function openModal(item = null) {
  ...
  if (item){ 
   ...
   form.value.division_id = item.upazila.division.id
   form.value.district_id = item.upazila.district_id
...
watch(() => form.value.district_id, (newVal, oldVal) => {
  if (oldVal){
...
<template>
      ...
      <div class="card card-outline card-info">
        ...
        <div class="card-body">
          <DataTable
            ...
            :fields="[
              { key: 'sl', label: 'SL' },
              { key: 'upazila.division.bn_name', label: 'বিভাগের নাম' },
              { key: 'upazila.district.bn_name', label: 'জেলার নাম' },
              { key: 'upazila.bn_name', label: 'উপজেলার নাম' },
              { key: 'name', label: 'ইউনিয়নের নাম ইংলিশ' },
              { key: 'bn_name', label: 'ইউনিয়নের নাম বাংলা' },
  ...
  >
    ...
    <BaseFormGroup label="জেলার নাম" labelCols="12" required>
     ...
     <BaseFormGroup label="উপজেলার নাম" labelCols="12" required>
       <ResourceSelect
              ...
              :labelField="(item) => `${item.bn_name}`"
              :isEdit="isEdit"
             :extraParams="form.district_id ? { district_id: form.district_id } : {}"
              
            />
    </BaseFormGroup>
    
    <BaseFormGroup label="ইউনিয়নের নাম বাংলা" labelCols="12" required>
      <BFormInput v-model="form.name" />
    </BaseFormGroup>
    <BaseFormGroup label="ইউনিয়নের নাম ইংলিশ" labelCols="12" required>
